...
function Player(options) {
        ...
        this.pointLoss = function(opponent) {
            if (opponent.paddle) {
                var totalRatingWithPaddle = opponent.rating + opponent.paddle.paddleRating; //add paddleRating to opponent's rating
                if (totalRatingWithPaddle > this.rating) { //if paddleRating and rating is greater than other player's rating
                    opponent.points = opponent.points + 1; // give them one point
                    alert(opponent.name + " won the point! The score is: " + this.points + "|" +
                        opponent.points);
                    console.log("line 23");
                }
            } else if (this.rating > opponent.rating) {
                this.points = this.points + 1;
                alert(this.name + " won the point! The score is: " + opponent.points + "|" +
                    this.points);
                console.log("line 29");
            } else {
                opponent.points = opponent.points + 1;
                alert(this.name + " lost the point! The score is: " + opponent.points + "|" +
                    this.points);
                console.log("line 34");
            }
        ...
        if (this.rating > 10) {
            alert("recalculate " + this.name +
                " attributes so that total for 'Rating' is 10. Right now the total is " + this.rating +
                ".");
        }
        ...
        this.locationChoice = function(name) {
            this.location = name;
        };
...
function Paddle(options) {
    ...
    if (this.paddleRating > 10) {
        alert("recalculate '" + this.name +
            "' attributes so that total for 'Rating' is 10. Right now the total is " + this
            .paddleRating + ".");
    }
}
